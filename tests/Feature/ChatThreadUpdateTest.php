...
class ChatThreadUpdateTest extends TestCase
{
    use RefreshDatabase;
    ...
    {
        $user = User::factory()->create();
        $thread = ChatThread::create(['user_id' => $user->id, 'subject' => 'Mixed thread']);
    ...
    {
        $user = User::factory()->create();
        $thread = ChatThread::create(['user_id' => $user->id, 'subject' => 'Bounced around']);
        $message = $this->createMessage($thread, 'inbound', 'inbox');
    ...
    {
        $user = User::factory()->create();
        $thread = ChatThread::create(['user_id' => $user->id, 'subject' => 'Thread state']);
        $message = $this->createMessage($thread, 'inbound', 'inbox');
    ...
    private function createMessage(ChatThread $thread, string $direction, string $folder): ChatMessage
    {
